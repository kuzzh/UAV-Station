...
namespace GMap.NET
{
   ...
   public partial class TilePrefetcher : Form
   {
      ...
      {
         this.label1.Text = "当前缓存层数(" + zoom + "): " + ((int)e.UserState).ToString() + " of " + all + ", 完成: " + e.ProgressPercentage.ToString() + "%";
      ...
      {
          if (MessageBox.Show("您是否真的想结束缓存进程?", "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes)
          {
              UserAborted = true;
              this.Close();
          }
